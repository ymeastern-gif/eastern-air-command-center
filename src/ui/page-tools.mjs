...
export const SOURCE_FILTER_DIMS=['project','source','confidence'];
...
export function lookupsFor(state){
  return {
    ...
    owner:Object.fromEntries((state.raw.people??[]).map(p=>[p.id,p.name])),
    ...
    source:Object.fromEntries((state.universe.source??[]).map(v=>[v,human(v)])),
    confidence:Object.fromEntries((state.universe.confidence??[]).map(v=>[v,human(v)])),
  };
}
