...
export function createStore(){
  let state={user:null,member:null,raw:null,records:[],intelligenceRecords:[],combinedRecords:[],schedule:[],sourceFilterRecords:[],changeFilterRecords:[],maps:{},errors:[],page:'today',projectScope:'all',viewState:{filters:{search:'',include:{},exclude:{},date:{}}},universe:{},combinedUniverse:{},scheduleUniverse:{},sourceUniverse:{},changeUniverse:{},loading:true};
  ...
  function rebuildDerived(){
    ...
    const records=buildCanonicalModel(state.raw,state.user.id);const intelligenceRecords=buildIntelligenceRecords(state.raw,state.user.id);const combinedRecords=[...records,...intelligenceRecords];const maps=canonicalMaps(state.raw);const schedule=(state.raw.scheduleMilestones??[]).map(r=>normalizeMilestone(r,maps.projects,maps.sources));
    ...
    state={...state,records,intelligenceRecords,combinedRecords,schedule,maps,universe,combinedUniverse,scheduleUniverse,sourceFilterRecords,sourceUniverse,changeFilterRecords,changeUniverse};
  }
  function load({user,member,raw,errors=[]}){state={...state,user,member,raw,errors,loading:false};rebuildDerived();emit()}function refreshRaw(raw,errors=[]){state={...state,raw,errors};rebuildDerived();emit()}
